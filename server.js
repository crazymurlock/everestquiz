...
let questions = [], gameOpen = false, gameStarted = false;
const players = {};
...
io.on('connection', socket => {
  ...
  socket.on('join', nick => {
    if (!gameOpen) return;
    players[socket.id] = { nickname: nick, level:1, correct:0, time:0, color: randomColor() };
    updatePlayerList();
  ...
  socket.on('disconnect', () => { delete players[socket.id]; updatePlayerList(); });
});
