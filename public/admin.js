...
fetch('/evergameadmin865/questions').then(r=>r.json()).then(data=>data.forEach(q=>{
  const li=document.createElement('li');li.textContent=`${q.order}. ${q.question}`;list.append(li);
}));
...
openBtn.onclick=()=>fetch('/evergameadmin865/open',{method:'POST'});
closeBtn.onclick=()=>fetch('/evergameadmin865/close',{method:'POST'});
startBtn.onclick=()=>fetch('/evergameadmin865/start',{method:'POST'}).then(r=>r.json()).then(()=>alert('Starting!'));
});