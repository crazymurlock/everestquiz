...
socket.on('playerList', list => {
  ...
  // Remove old
  Object.keys(circleEls).forEach(nick => {
    if (!list.find(p => p.nickname === nick)) {
      circleEls[nick].el.remove();
      delete circleEls[nick];
    }
  });
  // Create/update
  Object.values(levels).forEach(group => {
    group.forEach((p, idx) => {
      ...
      el.style.bottom = baseBottom + (p.level - 1) * dy + 'px';
    });
  });
});
