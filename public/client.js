...
socket.on('playerList',list=>{
  ...
  Object.values(levels).forEach(group=>group.forEach((p,i)=>{
    ...
    if(!ent){
      ...
      const nx=document.createElement('div');nx.className='nick';nx.textContent=p.nickname;
      el.append(c,nx);
      playersContainer.append(el);
      ent={el};circles[p.nickname]=ent;
    }
    ent.el.classList.toggle('self',p.nickname===self);
    const left=(i+1)/(group.length+1);
    const x=cw*left-14;
    const y=ch*(p.level/maxLevel);
    ent.el.style.transform=`translate3d(${x}px, -${y}px, 0)`;
  }));
});

// Question display using template
...
socket.on('answerResult',res=>{
  ...
  setTimeout(()=>{questionContainer.style.display='none';},1000);
});
