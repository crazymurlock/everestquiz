...
socket.on('countdown', n => {
  if (n>0) { cntNum.innerText = n; countdown.classList.add('show'); }
  else { countdown.classList.remove('show'); lobbyDiv.style.display = 'none'; gameDiv.style.display = 'block'; }
});
socket.on('playerList', list => {
  ...
  Object.values(levelMap).forEach(group => group.forEach((p,i) => {
    ...
    if (!entry) {
      ...
      const c = document.createElement('div'); c.className='circle'; c.innerText=p.nickname.charAt(0).toUpperCase(); c.style.background=p.color;
      const n = document.createElement('div'); n.className='nick'; n.innerText=p.nickname;
      el.append(c,n); container.append(el); entry={el}; circles[p.nickname]=entry;
    }
    entry.el.classList.toggle('self', p.nickname===self);
    const lf = (i+1)/(group.length+1);
    const x = cw*lf - 14;
...
socket.on('question', ({question,options}) => {
  ...
  options.forEach((o,i) => { const b=document.createElement('button'); b.className='option-btn'; b.innerText=o; b.onclick=()=>socket.emit('answer',i); opts.append(b); });
...
socket.on('gameOver', ({winner,stats}) => {
  confetti({particleCount:200,spread:120}); gameDiv.style.display='none'; resultDiv.style.display='flex';
  wText.innerText = `🏅 ${winner.nickname} — правильных: ${winner.correct}, время: ${Math.round(winner.time/1000)}с`;
