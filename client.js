...
socket.on('playerList',list=>{
  ...
  const levels={};
  list.forEach(p=>{ if(!levels[p.level]) levels[p.level]=[]; levels[p.level].push(p); });
  Object.values(levels).forEach(group=>{
    group.forEach((p,idx)=>{
      ...
      if(!entry){
        ...
        el.append(circle,nickEl); playersContainer.append(el);
        entry={el,circle,nickEl}; circleEls[p.nickname]=entry;
      }
